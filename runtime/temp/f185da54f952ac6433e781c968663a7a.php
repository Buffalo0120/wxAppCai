<?php /*a:2:{s:67:"D:\project\erha\wxAppcai\themes/pc/miniapp/guess_question\list.html";i:1551513687;s:59:"D:\project\erha\wxAppcai\themes/pc/miniapp/common\base.html";i:1551446749;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    ...
    <div class="layui-form-item">
        <div class="layui-inline">
            ...
            <div class="layui-input-block">
                <input type="text" name="title" value="<?php echo htmlentities($post['title']); ?>" placeholder="请输入标题" autocomplete="off" class="layui-input">
            </div>
        </div>
        <div class="layui-inline">
            <div class="layui-input-block">
        ...
        <div class="layui-inline">
            <div class="layui-input-block">
                <a href="<?php echo url('guess_question/add'); ?>" class="layui-btn layui-btn-sm layui-btn-normal"  >添加</a>
            </div>
        </div>
    </div>
</form>
    ...
    <form class="layui-form" action="" style="padding: 0px 10px 0px 10px;">
        <table class="layui-table" lay-skin="line">
            ...
            <tr>
                ...
                <td><?php echo htmlentities($v['coin_pool']); ?></td>
                <td><?php echo htmlentities(date("Y-m-d H:i:s",!is_numeric($v['start_time'])? strtotime($v['start_time']) : $v['start_time'])); ?></td>
                <td><?php echo htmlentities(date("Y-m-d H:i:s",!is_numeric($v['stop_time'])? strtotime($v['stop_time']) : $v['stop_time'])); ?></td>
                <td><?php echo htmlentities($v['update_time']); ?></td>
                <td>
                    <a href="<?php echo url('guess_option/index',['q_id'=>$v['id']]); ?>"
                       class="layui-btn layui-btn-sm layui-btn-normal">查看</a>
                </td>
                <td>
                    <a href="<?php echo url('guess_question/edit',['id'=>$v['id']]); ?>"
...
<script>
    ...
    function aa(id)
    {
        layer.confirm('确定删除吗？',{btn:['确定','取消']},function(res){
            $.post("<?php echo url('guess_question/del'); ?>",{id:id},function (res) {
                if(res.status=='0'){
                    layer.msg(res.msg);
                }
                if(res.status=='1'){
