...
class Miniapp extends Base
{
    ...
    public function userList()
    {
        ...
        if (Request::isGet()) {
            ...
            if (isset($_data['nickname'])) {
                $where[] = ['mu.nickname', 'like', "%{$_data['nickname']}%"];
            }
            if (isset($_data['times'])) {
                $times = explode('到', $_data['times']);
                if (count($times) == 2) {
                    $where[] = ['mu.create_time', 'between', [strtotime($times[0]), strtotime($times[1]) + 86400]];
                }
                $times = $_data['times'];
            }
            if (isset($_data['need']) && $_data['need'] == 1) {

                $where[] = ['mu.last_time', '>', (time() - (86400 * 2))];
                $need = 1;
            }
            if (isset($_data['sex']) && $_data['sex'] != 0) {
                $sex = $_data['sex'] ? $_data['sex'] : '0';
                $where[] = ['mu.gender', '=', $_data['sex']];
            }
        }
        ...
        $users = $model->where('mu.mpid', '=', $this->_mid)
            ->alias('mu')
     ...
     */
    public function setting($type = 'systemconf')
    {
        if (Request::isAjax()) {
            $input = input('post.');
